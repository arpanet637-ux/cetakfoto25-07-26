...
const UpdateStoreSettingsSchema = StoreSettingsSchema.omit({
  id: true,
  created_at: true,
  updated_at: true
}).partial();
...
app$b.use("/*", authMiddleware);
app$b.get("/export", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const products = await db.prepare("SELECT name, price, stock, min_stock, default_method FROM products WHERE user_id = ? ORDER BY name ASC").bind(user.id).all();
  const exportData = {
...
app$b.post("/import", async (c) => {
  ...
  const ProductImportSchema = objectType({
    name: stringType().min(1),
    price: numberType().min(0),
    stock: numberType().min(0),
    min_stock: numberType().min(0),
    default_method: enumType(["cetak_sendiri", "tim_produksi"])
  });
  ...
  for (const product of products) {
    ...
    const existing = await db.prepare("SELECT id FROM products WHERE name = ? AND user_id = ?").bind(name, user.id).first();
    if (existing) {
      await db.prepare(
        `UPDATE products SET price = ?, stock = ?, min_stock = ?, default_method = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`
      ).bind(price, stock, min_stock, default_method, existing.id).run();
    } else {
      await db.prepare(
        `INSERT INTO products (name, price, stock, min_stock, default_method, user_id) VALUES (?, ?, ?, ?, ?, ?)`
      ).bind(name, price, stock, min_stock, default_method, user.id).run();
    }
    imported++;
  }
  return c.json({
...
app$b.get("/", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const products = await db.prepare("SELECT * FROM products WHERE user_id = ? ORDER BY name ASC").bind(user.id).all();
  return c.json(products.results);
});
app$b.get("/:id", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const id = c.req.param("id");
  const product = await db.prepare("SELECT * FROM products WHERE id = ? AND user_id = ?").bind(id, user.id).first();
  if (!product) {
...
app$b.put("/:id", async (c) => {
  ...
  const existing = await db.prepare("SELECT * FROM products WHERE id = ? AND user_id = ?").bind(id, user.id).first();
  if (!existing) {
    return c.json({ error: "Product not found" }, 404);
  }
  ...
  if (updates.min_stock !== void 0) {
    fields.push("min_stock = ?");
    values.push(updates.min_stock);
  }
  if (updates.default_method !== void 0) {
...
app$b.get("/alerts/low-stock", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const products = await db.prepare("SELECT * FROM products WHERE user_id = ? AND stock <= min_stock ORDER BY stock ASC").bind(user.id).all();
  return c.json(products.results);
});
const app$a = new Hono2();
app$a.use("/*", authMiddleware);
...
app$a.get("/export", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const orders = await db.prepare(`SELECT order_number, client_name, client_phone, client_address, notes, total_amount, paid_amount, created_at FROM orders WHERE user_id = ? ORDER BY created_at DESC`).bind(user.id).all();
  const ordersWithItems = await Promise.all(
    orders.results.map(async (order) => {
      const items = await db.prepare(`SELECT product_name, quantity, unit_price, discount, subtotal, method, deadline_date, status_work, status_send, status_payment, status_pickup FROM order_items WHERE order_id = (SELECT id FROM orders WHERE order_number = ? AND user_id = ?) ORDER BY id ASC`).bind(order.order_number, user.id).all();
...
app$a.post("/import", async (c) => {
  ...
  const OrderItemImportSchema = objectType({
    product_name: stringType().min(1),
    quantity: numberType().min(1),
    unit_price: numberType().min(0),
    discount: numberType().min(0),
    subtotal: numberType().min(0),
    method: enumType(["cetak_sendiri", "tim_produksi"]),
    deadline_date: stringType(),
    status_work: stringType().nullable().optional(),
    status_send: stringType().nullable().optional(),
    status_payment: stringType().nullable().optional(),
    status_pickup: stringType().nullable().optional()
  });
  const OrderImportSchema = objectType({
    order_number: stringType().min(1),
    client_name: stringType().min(1),
    client_phone: stringType().nullable().optional(),
    client_address: stringType().nullable().optional(),
    notes: stringType().nullable().optional(),
    total_amount: numberType().min(0),
    paid_amount: numberType().min(0),
    created_at: stringType().optional(),
    items: arrayType(OrderItemImportSchema).optional()
  });
  ...
  for (const order of orders) {
    ...
    const existing = await db.prepare("SELECT id FROM orders WHERE order_number = ? AND user_id = ?").bind(data.order_number, user.id).first();
    let orderId;
    if (existing) {
      await db.prepare(
      ...
      ).bind(data.client_name, data.client_phone || null, data.client_address || null, data.notes || null, data.total_amount, data.paid_amount, existing.id).run();
      orderId = existing.id;
      ...
      ).bind(data.order_number, data.client_name, data.client_phone || null, data.client_address || null, data.notes || null, data.total_amount, data.paid_amount, user.id, data.created_at || (/* @__PURE__ */ new Date()).toISOString()).run();
    ...
    if (data.items && data.items.length > 0) {
      for (const item of data.items) {
        ...
        ).bind(orderId, item.product_name, item.quantity, item.unit_price, item.discount, item.subtotal, item.method, item.deadline_date, item.status_work || null, item.status_send || null, item.status_payment || null, item.status_pickup || null, user.id).run();
      }
    }
    imported++;
  }
...
app$a.get("/", async (c) => {
  ...
  const ordersWithItems = await Promise.all(
    orders.results.map(async (order) => {
      const items = await db.prepare("SELECT * FROM order_items WHERE order_id = ? ORDER BY id ASC").bind(order.id).all();
      return { ...order, items: items.results };
    })
  );
...
app$a.get("/:id", async (c) => {
  ...
  const items = await db.prepare("SELECT * FROM order_items WHERE order_id = ? ORDER BY id ASC").bind(id).all();
...
app$a.post("/", async (c) => {
  ...
  const { client_name, client_phone, client_address, notes, discount, branch_id, total_amount, paid_amount } = parsed.data;
  const result = await db.prepare(
  ...
  const newOrder = await db.prepare("SELECT * FROM orders WHERE id = ?").bind(result.meta.last_row_id).first();
  return c.json({ ...newOrder, items: [] }, 201);
});
app$a.put("/:id", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const id = c.req.param("id");
  const body = await c.req.json();
  ...
  const existing = await db.prepare("SELECT * FROM orders WHERE id = ? AND user_id = ?").bind(id, user.id).first();
  if (!existing) {
    return c.json({ error: "Order not found" }, 404);
  }
  const updates = parsed.data;
...
app$a.delete("/:id", async (c) => {
  ...
  const items = await db.prepare("SELECT product_id, quantity FROM order_items WHERE order_id = ?").bind(id).all();
...
app$a.post("/:orderId/items", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const orderId = c.req.param("orderId");
  const order = await db.prepare("SELECT id FROM orders WHERE id = ? AND user_id = ?").bind(orderId, user.id).first();
  if (!order) {
    return c.json({ error: "Order not found" }, 404);
  }
  const body = await c.req.json();
  ...
  const newItem = await db.prepare("SELECT * FROM order_items WHERE id = ?").bind(result.meta.last_row_id).first();
  return c.json(newItem, 201);
});
app$a.put("/:orderId/items/:itemId", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const orderId = c.req.param("orderId");
  ...
  const existing = await db.prepare("SELECT * FROM order_items WHERE id = ? AND order_id = ?").bind(itemId, orderId).first();
  if (!existing) {
  ...
  if (fields.length > 0) {
    ...
    await db.prepare(`UPDATE order_items SET ${fields.join(", ")} WHERE id = ?`).bind(...values).run();
    if (updates.quantity !== void 0 && existing.product_id) {
      const oldQuantity = existing.quantity;
      const newQuantity = updates.quantity;
      const quantityDiff = oldQuantity - newQuantity;
      await adjustProductStock(db, existing.product_id, quantityDiff);
    }
    if (updates.quantity !== void 0 || updates.unit_price !== void 0 || updates.discount !== void 0 || updates.subtotal !== void 0) {
      await recalculateOrderTotal(db, Number(orderId));
    }
  }
...
app$a.delete("/:orderId/items/:itemId", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const orderId = c.req.param("orderId");
  const itemId = c.req.param("itemId");
  ...
  if (existing.product_id) {
    await adjustProductStock(db, existing.product_id, existing.quantity);
  }
  await db.prepare("DELETE FROM order_items WHERE id = ?").bind(itemId).run();
  await recalculateOrderTotal(db, Number(orderId));
  return c.json({ success: true });
});
async function recalculateOrderTotal(db, orderId) {
  const items = await db.prepare("SELECT subtotal FROM order_items WHERE order_id = ?").bind(orderId).all();
  const itemsTotal = items.results.reduce(
    (sum, item) => sum + (Number(item.subtotal) || 0),
    0
  );
  const order = await db.prepare("SELECT discount FROM orders WHERE id = ?").bind(orderId).first();
  const orderDiscount = Number(order?.discount) || 0;
  const total = Math.max(0, itemsTotal - orderDiscount);
  await db.prepare("UPDATE orders SET total_amount = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(total, orderId).run();
}
async function adjustProductStock(db, productId, quantityChange) {
  if (!productId) return;
  await db.prepare("UPDATE products SET stock = stock + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(quantityChange, productId).run();
}
const app$9 = new Hono2();
app$9.use("/*", authMiddleware);
app$9.get("/", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
app$9.get("/:id", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const id = c.req.param("id");
  const expense = await db.prepare("SELECT * FROM expenses WHERE id = ? AND user_id = ?").bind(id, user.id).first();
  if (!expense) {
...
app$9.post("/", async (c) => {
  ...
  const { expense_type, description, amount, expense_date, vendor_name, order_id } = parsed.data;
  const result = await db.prepare(
...
app$9.put("/:id", async (c) => {
  ...
  const existing = await db.prepare("SELECT * FROM expenses WHERE id = ? AND user_id = ?").bind(id, user.id).first();
  if (!existing) {
    return c.json({ error: "Expense not found" }, 404);
  }
  const updates = parsed.data;
...
app$9.get("/summary/total", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
app$8.use("/*", authMiddleware);
app$8.get("/", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  let settings = await db.prepare("SELECT * FROM store_settings WHERE user_id = ? LIMIT 1").bind(user.id).first();
  if (!settings) {
    await db.prepare(
      `INSERT INTO store_settings (name, address, phone, email, instagram, facebook, user_id)
         VALUES ('Nama Toko', '', '', '', '', '', ?)`
    ).bind(user.id).run();
    settings = await db.prepare("SELECT * FROM store_settings WHERE user_id = ? LIMIT 1").bind(user.id).first();
  }
...
app$8.put("/", async (c) => {
  ...
  if (fields.length > 0) {
    fields.push("updated_at = CURRENT_TIMESTAMP");
    values.push(String(settings.id));
    await db.prepare(`UPDATE store_settings SET ${fields.join(", ")} WHERE id = ?`).bind(...values).run();
  }
  const updated = await db.prepare("SELECT * FROM store_settings WHERE user_id = ? LIMIT 1").bind(user.id).first();
  return c.json(updated);
});
const app$7 = new Hono2();
app$7.use("/*", authMiddleware);
app$7.get("/stats", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  ...
  return c.json({
    totalActiveOrders: Number(activeOrders?.count) || 0,
    pendingVendor: Number(pendingVendor?.count) || 0,
    deadlineToday: Number(deadlineToday?.count) || 0,
    unpaidOrders: Number(unpaidOrders?.count) || 0
  });
});
app$7.get("/notifications/deadline", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
...
app$7.get("/notifications/vendor", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const notifications = await db.prepare(
    `SELECT oi.id, oi.order_id, o.order_number, o.client_name, oi.product_name,
...
app$7.get("/notifications/payment-status", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const notifications = await db.prepare(
...
app$7.get("/notifications/stock", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const notifications = await db.prepare(
...
app$6.get("/oauth/google/redirect_url", async (c) => {
...
app$6.get("/users/me", authMiddleware, async (c) => {
  return c.json(c.get("user"));
});
app$6.get("/logout", async (c) => {
  const sessionToken = getCookie(c, MOCHA_SESSION_TOKEN_COOKIE_NAME);
  if (typeof sessionToken === "string") {
...
app$5.get("/check/:orderNumber", async (c) => {
  const db = c.env.DB;
  ...
  const storeSettings = await db.prepare("SELECT name, phone, address FROM store_settings LIMIT 1").first();
  return c.json({
...
app$4.use("/*", authMiddleware);
app$4.get("/", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const branches = await db.prepare("SELECT * FROM branches WHERE user_id = ? ORDER BY name ASC").bind(user.id).all();
  return c.json(branches.results);
});
app$4.get("/:id", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const id = parseInt(c.req.param("id"));
  const branch = await db.prepare("SELECT * FROM branches WHERE id = ? AND user_id = ?").bind(id, user.id).first();
  if (!branch) {
...
app$4.post("/", async (c) => {
  ...
  ).bind(name, address || null, phone || null, user.id).run();
  const newBranch = await db.prepare("SELECT * FROM branches WHERE id = ?").bind(result.meta.last_row_id).first();
  return c.json(newBranch, 201);
});
app$4.put("/:id", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const id = parseInt(c.req.param("id"));
  const body = await c.req.json();
  ...
  const existing = await db.prepare("SELECT * FROM branches WHERE id = ? AND user_id = ?").bind(id, user.id).first();
  if (!existing) {
    return c.json({ error: "Branch not found" }, 404);
  }
  const { name, address, phone } = parsed.data;
  ...
  const updated = await db.prepare("SELECT * FROM branches WHERE id = ?").bind(id).first();
  return c.json(updated);
});
app$4.delete("/:id", async (c) => {
  ...
  await db.prepare("UPDATE orders SET branch_id = NULL WHERE branch_id = ? AND user_id = ?").bind(id, user.id).run();
  await db.prepare("DELETE FROM branches WHERE id = ? AND user_id = ?").bind(id, user.id).run();
  return c.json({ success: true });
});
const app$3 = new Hono2();
app$3.use("/*", authMiddleware);
app$3.post("/pickup/:orderId", async (c) => {
  const user = c.get("user");
  const orderId = c.req.param("orderId");
  const db = c.env.DB;
  const order = await db.prepare("SELECT id FROM orders WHERE id = ? AND user_id = ?").bind(orderId, user.id).first();
...
app$3.post("/invoice-screenshot", async (c) => {
  const user = c.get("user");
  const formData = await c.req.formData();
...
app$2.use("/*", authMiddleware);
const getUser = (c) => c.get("user");
app$2.get("/notifications", async (c) => {
...
app$2.get("/order/:orderId", async (c) => {
  const user = getUser(c);
...
app$1.post("/notification", async (c) => {
  const db = c.env.DB;
  try {
    ...
    const acquirerData = body.acquirer;
    const channelData = body.channel;
    const invoiceNumber = orderData?.invoice_number || body.invoice_number || orderData?.order_id || body.order_id || transactionData?.original_request_id;
    const transactionStatus = (transactionData?.status || body.transaction_status || body.status || body.result || "").toUpperCase();
    const channelId = channelData?.id || acquirerData?.id || body.payment_channel || "DOKU";
    const channelName = channelData?.name || acquirerData?.name || channelId;
    ...
    let dokuPayment = await db.prepare("SELECT * FROM doku_payments WHERE invoice_number = ? AND status = 'pending' LIMIT 1").bind(invoiceNumber).first();
    if (!dokuPayment) {
      console.log("No exact match, trying partial match for:", invoiceNumber);
      dokuPayment = await db.prepare("SELECT * FROM doku_payments WHERE invoice_number LIKE ? AND status = 'pending' LIMIT 1").bind(`%${invoiceNumber}%`).first();
      if (!dokuPayment) {
        console.log("No pending payment found for invoice:", invoiceNumber);
        const allPending = await db.prepare("SELECT invoice_number FROM doku_payments WHERE status = 'pending' LIMIT 10").all();
    ...
    if (isSuccess) {
      await db.prepare("UPDATE doku_payments SET status = 'success', payment_method = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(channelName, dokuPayment.id).run();
      console.log("Updated doku_payment status to success");
      const order = await db.prepare("SELECT * FROM orders WHERE id = ?").bind(dokuPayment.order_id).first();
      console.log("Order found:", order ? order.order_number : "NOT FOUND");
      if (order) {
        const newPaidAmount = order.paid_amount + dokuPayment.amount;
        await db.prepare("UPDATE orders SET paid_amount = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(newPaidAmount, order.id).run();
        ...
        await db.prepare(`
            INSERT INTO payment_records (order_id, amount, payment_method, payment_type, user_id, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
          `).bind(dokuPayment.order_id, dokuPayment.amount, paymentMethodDisplay, paymentType, dokuPayment.user_id, now, now).run();
        ...
        if (order.client_phone) {
          try {
            const storeSettings = await db.prepare("SELECT name FROM store_settings WHERE user_id = ? LIMIT 1").bind(dokuPayment.user_id).first();
...
`;
      ...
      await db.prepare("UPDATE doku_payments SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(transactionStatus.toLowerCase(), dokuPayment.id).run();
...
app$1.post("/webhook", async (c) => {
  ...
  try {
    ...
    const dokuPayment = await db.prepare("SELECT * FROM doku_payments WHERE invoice_number = ? AND status = 'pending' LIMIT 1").bind(invoiceNumber).first();
    if (!dokuPayment) {
      console.log("No pending payment found for invoice:", invoiceNumber);
      return c.json({ message: "Payment not found or already processed" }, 200);
    }
    if (transactionStatus === "SUCCESS" || transactionStatus === "PAID") {
      await db.prepare("UPDATE doku_payments SET status = 'success', payment_method = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(channelId || "DOKU", dokuPayment.id).run();
      const order = await db.prepare("SELECT * FROM orders WHERE id = ?").bind(dokuPayment.order_id).first();
      if (order) {
        const newPaidAmount = order.paid_amount + dokuPayment.amount;
        await db.prepare("UPDATE orders SET paid_amount = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(newPaidAmount, order.id).run();
        ...
        await db.prepare(`
            INSERT INTO payment_records (order_id, amount, payment_method, payment_type, user_id, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
          `).bind(dokuPayment.order_id, dokuPayment.amount, "Transfer (DOKU)", paymentType, dokuPayment.user_id, now, now).run();
...
app$1.use("/*", authMiddleware);
app$1.get("/", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  let settings = await db.prepare("SELECT * FROM payment_gateway_settings WHERE user_id = ? LIMIT 1").bind(user.id).first();
  if (!settings) {
    return c.json({
      has_pin: false,
      doku_client_id: null,
...
app$1.post("/verify-pin", async (c) => {
  ...
  const settings = await db.prepare("SELECT * FROM payment_gateway_settings WHERE user_id = ? LIMIT 1").bind(user.id).first();
  ...
  return c.json({
    verified: true,
    doku_client_id: settings.doku_client_id ? decryptData(settings.doku_client_id, user.id) : "",
    doku_secret_key: settings.doku_secret_key ? decryptData(settings.doku_secret_key, user.id) : "",
    doku_environment: settings.doku_environment || "sandbox"
  });
});
app$1.put("/", async (c) => {
  ...
  const updates = [];
  const values = [];
  if (parsed.data.doku_client_id !== void 0) {
    updates.push("doku_client_id = ?");
    values.push(
      parsed.data.doku_client_id ? encryptData(parsed.data.doku_client_id, user.id) : null
    );
  }
  if (parsed.data.doku_secret_key !== void 0) {
    updates.push("doku_secret_key = ?");
    values.push(
      parsed.data.doku_secret_key ? encryptData(parsed.data.doku_secret_key, user.id) : null
    );
  }
  if (parsed.data.doku_environment !== void 0) {
...
app$1.post("/create-link", async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const body = await c.req.json();
  ...
  const order = await db.prepare("SELECT * FROM orders WHERE id = ? AND user_id = ?").bind(order_id, user.id).first();
  if (!order) {
    return c.json({ error: "Pesanan tidak ditemukan" }, 404);
  }
  ...
  const storeSettings = await db.prepare("SELECT * FROM store_settings WHERE user_id = ? LIMIT 1").bind(user.id).first();
  ...
  const apiUrl = environment === "production" ? "https://api.doku.com/checkout/v1/payment" : "https://api-sandbox.doku.com/checkout/v1/payment";
  ...
  const requestBody = {
    ...
    customer: {
      ...
      phone: order.client_phone || void 0
    }
  };
  ...
  try {
    ...
    await db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, 'pending', ?)
      `).bind(order_id, invoiceNumber, remainingAmount, adminFee, totalWithFee, user.id).run();
    return c.json({
      success: true,
...
app.use("/*", cors());
app.route("/api", app$6);
app.route("/api/products", app$b);
app.route("/api/orders", app$a);
app.route("/api/expenses", app$9);
app.route("/api/settings", app$8);
app.route("/api/dashboard", app$7);
app.route("/api/queue", app$5);
app.route("/api/branches", app$4);
app.route("/api/uploads", app$3);
app.route("/api/payments", app$2);
app.route("/api/payment-gateway", app$1);
