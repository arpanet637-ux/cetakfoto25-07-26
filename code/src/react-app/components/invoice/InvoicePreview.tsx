  ...
  const handleSendWhatsApp = async () => {
    ...
    try {
      ...
      if (remainingAmount > 0) {
        ...
        // Try to create DOKU payment link
        try {
          const dokuResponse = await fetch("/api/payment-gateway/create-link", {
            method: "POST",
          ...
          if (dokuResponse.ok && dokuResult.payment_url) {
            message += `\n💳 *Link Pembayaran:*\n${dokuResult.payment_url}\n`;
          }
        } catch (err) {
          console.log("DOKU error:", err);
        }
      } else {
        message += `\n✅ *LUNAS*\n`;
      }
